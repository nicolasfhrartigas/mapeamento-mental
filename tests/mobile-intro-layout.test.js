...
test('mobile mantém o texto original da intro (revertido de uma reescrita não autorizada)', async () => {
  const source = await readFile('src/mobile.dc.html', 'utf8');

  assert.match(source, /Todo\s+atleta conhece o próprio corpo em detalhe: carga, pace, medidas, tempo de prova\./);
  assert.match(source, /São dez perguntas fixas e uma\s+pergunta contextual para sua modalidade\./);
  assert.doesNotMatch(source, /Você acompanha carga, pace, medidas e tempo de prova\./);
  assert.doesNotMatch(source, /class="mobile-intro-guide"/);
  assert.doesNotMatch(source, /Como funciona/);
});
